...
class Actor {
     ...
     */
    authenticationUrl() {
        return this.authenticationEmail().then((email) => getUrls(email.text).values().next().value);
    }
    /**
    ...
    async authenticationCode() {
        return this.authenticationEmail().then((email) => email.text.match(/password is (\d+)/)[1]);
    }
    /**
     * @returns {Promise<MailServerEmail[]>}
     */
    authenticationEmail() {
        return this.emails({ text: (t) => t.match(/password is (\d+)/) }).then(last);
    }
    /**
     * @param {ThenableWebDriver} driver
