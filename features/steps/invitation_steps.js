...
import { assertDisplayed, refuteDisplayed } from "../support/assertDisplayed.js";
const assert = assert$0.strict;
When("an {invitation} to {a} {space} is sent by {actor}", { timeout: 90000 }, function (invitation, _, space, actor, invitations) {
    /**
...
Then("an {invitation} for {a} {space} is delivered",
/**
 * @param {Invitation} invitation
 */
...
Then("{a} {invitation} for {a} {space} has {a} status of {string}",
/**
 * @param {Invitation} invitation
 */
...
function (_a, actor, _a2, _role, _a3, space) {
    return (actor
        ...
        .then(() => assertDisplayed(new Component(this.driver, '*[aria-label="Configure Space"]'))));
});
Then('{a} {actor} does not become {a} {actor} of {a} {space}', function (_a, actor, _a2, _role, _a3, space) {
    ...
    return (actor
        .signIn(this.driver, space)
        // @todo Refactor to live in the harness
        .then(() => refuteDisplayed(new Component(this.driver, 'header a.--configure[aria_label="Configure Space"]'))));
});
Then("all other Invitations to {actor} for {a} {space} no longer have {a} status of {string}",
/**
 * @param {Actor} actor
 * @param {Space} space
 * @param {string} status
...
function (actor, _a, space, _a2, status) {
    return actor
        ...
        .then((invitations) => assert(invitations.every((i) => i.status !== status)));
});
