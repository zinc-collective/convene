...
Given("an unauthenticated {actor} has requested to be identified via Email", async function (actor) {
    this.actor = actor;
...
Given("{a} {actor} is signed in to {a} {space}", function (a, actor, a2, space) {
    return actor
        .signOut(this.driver)
...
When("the unauthenticated {actor} opens the Identification Verification Link emailed to them",
/** @param {Actor} actor */
function (actor) {
...
Then("the {actor} is Verified as the Owner of that Email Address", async function (actor) {
